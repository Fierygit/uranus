...
NC_TIMEOUT=3
ERROR_RETRY_TIMES=3
START_RETYR_TIMES=$[1 * 3]     # 5 minites
START_COORDINATOR_ONLY=1
START_COORDINATOR_AND_ALL_PARTICIPANTS=5
...
{
	 sudo  ifconfig lo:0 192.168.66.101/24
	 sudo  ifconfig lo:1 192.168.66.201/24
	 sudo  ifconfig lo:2 192.168.66.202/24
	 sudo  ifconfig lo:3 192.168.66.203/24
}
...
{
	  sudo  tc qdisc add dev lo:0 root netem delay ${DELAY}ms
	  sudo  tc qdisc add dev lo:1 root netem delay ${DELAY}ms
	  sudo  tc qdisc add dev lo:2 root netem delay ${DELAY}ms
	  sudo  tc qdisc add dev lo:3 root netem delay ${DELAY}ms
}

function set_virtual_nics_packet_loss
{
	  sudo  tc qdisc add dev lo:0 root netem loss ${PACKET_LOSS_RATE}%
	  sudo  tc qdisc add dev lo:1 root netem loss ${PACKET_LOSS_RATE}%
	  sudo  tc qdisc add dev lo:2 root netem loss ${PACKET_LOSS_RATE}%
	  sudo  tc qdisc add dev lo:3 root netem loss ${PACKET_LOSS_RATE}%
}
